...
_aa._ab = function(f, e) {
    var d = codigoqr.width;
    var b = codigoqr.height;
    var c = true;
...
_aa._af = function(b, d, a) {
    ...
    for (var f = 0; f < d; f++) {
        ...
        try {
            for (var h = 0; h < g; h += 2) {
                var e = b[Math.floor(j[h]) + codigoqr.width * Math.floor(j[h + 1])];
                if (e) {
                    k._dq(h >> 1, f)
                }
            }
...
function Detector(a) {
    ...
    this._bi = function(m, l, c, b) {
        ...
        for (var h = m, g = l; h != c; h += f) {
            ...
            var s = d ? h : g;
            if (e == 1) {
                if (this.image[t + s * codigoqr.width]) {
                    e++
                }
            } else {
                if (!this.image[t + s * codigoqr.width]) {
    ...
    this._bh = function(i, g, h, f) {
        ...
        if (c < 0) {
            ...
            if (c >= codigoqr.height) {
                e = (codigoqr.height - 1 - g) / (c - g);
                c = codigoqr.height - 1
            }
        }
    ...
    this._bl = function(g, f, d, j) {
        ...
        var i = Math.min(codigoqr.width - 1, f + k);
        if (i - h < g * 3) {
            throw "Error"
        }
        var b = Math.max(0, d - k);
        var c = Math.min(codigoqr.height - 1, d + k);
...
var codigoqr = {};
codigoqr.imagedata = null;
codigoqr.width = 0;
codigoqr.height = 0;
codigoqr.qrCodeSymbol = null;
codigoqr.debug = false;
codigoqr.maxImgSize = 1024 * 1024;
codigoqr._eo = [
    [10, 9, 8, 8],
    [12, 11, 16, 10],
    [14, 13, 16, 12]
];
codigoqr.callback = null;
codigoqr.vidSuccess = function(a) {
    codigoqr.localstream = a;
    if (codigoqr.webkit) {
        codigoqr.video.src = window.webkitURL.createObjectURL(a)
    } else {
        if (codigoqr.moz) {
            codigoqr.video.mozSrcObject = a;
            codigoqr.video.play()
        } else {
            codigoqr.video.src = a
        }
    }
    codigoqr.gUM = true;
    codigoqr.canvas_qr2 = document.createElement("canvas");
    codigoqr.canvas_qr2.id = "qr-canvas";
    codigoqr.qrcontext2 = codigoqr.canvas_qr2.getContext("2d");
    codigoqr.canvas_qr2.width = codigoqr.video.videoWidth;
    codigoqr.canvas_qr2.height = codigoqr.video.videoHeight;
    setTimeout(codigoqr.captureToCanvas, 500)
};
codigoqr.vidError = function(a) {
    codigoqr.gUM = false;
    return
};
codigoqr.captureToCanvas = function() {
    if (codigoqr.gUM) {
        try {
            if (codigoqr.video.videoWidth == 0) {
                setTimeout(codigoqr.captureToCanvas, 500);
                return
            } else {
                codigoqr.canvas_qr2.width = codigoqr.video.videoWidth;
            ...
            codigoqr.qrcontext2.drawImage(codigoqr.video, 0, 0);
            try {
                codigoqr.decode()
            } catch (a) {
                console.log(a);
...
codigoqr.setWebcam = function(c) {
    var d = navigator;
    codigoqr.video = document.getElementById(c);
    var a = true;
    ...
    if (d.getUserMedia) {
        d.getUserMedia({
            video: a,
            audio: false
        }, codigoqr.vidSuccess, codigoqr.vidError)
    } else {
        if (d.webkitGetUserMedia) {
            codigoqr.webkit = true;
...
codigoqr.decode = function(d) {
    if (arguments.length == 0) {
        if (codigoqr.canvas_qr2) {
            var b = codigoqr.canvas_qr2;
            var a = codigoqr.qrcontext2
        } else {
            var b = document.getElementById("qr-canvas");
            var a = b.getContext("2d")
        }
        codigoqr.width = b.width;
        codigoqr.height = b.height;
        codigoqr.imagedata = a.getImageData(0, 0, codigoqr.width, codigoqr.height);
        codigoqr.result = codigoqr.process(a);
        if (codigoqr.callback != null) {
            codigoqr.callback(codigoqr.result)
        }
        return codigoqr.result
    } else {
        var c = new Image();
        ...
        c.onload = function() {
            ...
            var f = c.height;
            var l = c.width;
            if (c.width * c.height > codigoqr.maxImgSize) {
                var k = c.width / c.height;
                f = Math.sqrt(codigoqr.maxImgSize / k);
                l = k * f
            }
            i.width = l;
            i.height = f;
            h.drawImage(c, 0, 0, i.width, i.height);
            codigoqr.width = i.width;
            codigoqr.height = i.height;
            try {
                codigoqr.imagedata = h.getImageData(0, 0, i.width, i.height)
            } catch (m) {
                codigoqr.result = "Cross domain image reading not supported in your browser! Save it to your computer then drag and drop the file!";
                if (codigoqr.callback != null) {
                    codigoqr.callback(codigoqr.result)
                }
                return
            ...
            try {
                codigoqr.result = codigoqr.process(h)
            } catch (m) {
                console.log(m);
                codigoqr.result = "error decoding QR Code"
            }
            if (codigoqr.callback != null) {
                codigoqr.callback(codigoqr.result)
            }
        ...
        c.onerror = function() {
            if (codigoqr.callback != null) {
                codigoqr.callback("Failed to load the image")
            }
        };
        c.src = d
    }
};
codigoqr.isUrl = function(a) {
    var b = /(ftp|http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/;
    return b.test(a)
};
codigoqr.decode_url = function(b) {
    var d = "";
    try {
        d = escape(b)
    } catch (c) {
...
codigoqr.decode_utf8 = function(a) {
    if (codigoqr.isUrl(a)) {
        return codigoqr.decode_url(a)
    } else {
        return a
    }
};
codigoqr.process = function(q) {
    var a = new Date().getTime();
    var c = codigoqr.grayScaleToBitmap(codigoqr.grayscale());
    if (codigoqr.debug) {
        for (var m = 0; m < codigoqr.height; m++) {
            for (var n = 0; n < codigoqr.width; n++) {
                var o = (n * 4) + (m * codigoqr.width * 4);
                codigoqr.imagedata.data[o] = c[n + m * codigoqr.width] ? 0 : 0;
                codigoqr.imagedata.data[o + 1] = c[n + m * codigoqr.width] ? 0 : 0;
                codigoqr.imagedata.data[o + 2] = c[n + m * codigoqr.width] ? 255 : 0
            }
        }
        q.putImageData(codigoqr.imagedata, 0, 0)
    }
    ...
    if (codigoqr.debug) {
        for (var m = 0; m < p.bits.Height; m++) {
            for (var n = 0; n < p.bits.Width; n++) {
                var o = (n * 4 * 2) + (m * 2 * codigoqr.width * 4);
                codigoqr.imagedata.data[o] = p.bits._ds(n, m) ? 0 : 0;
                codigoqr.imagedata.data[o + 1] = p.bits._ds(n, m) ? 0 : 0;
                codigoqr.imagedata.data[o + 2] = p.bits._ds(n, m) ? 255 : 0
            }
        }
        q.putImageData(codigoqr.imagedata, 0, 0)
    }
    ...
    var b = d - a;
    console.log(b);
    return codigoqr.decode_utf8(l)
};
codigoqr.getPixel = function(b, d) {
    if (codigoqr.width < b) {
        throw "point error"
    }
    if (codigoqr.height < d) {
        throw "point error"
    }
    var a = (b * 4) + (d * codigoqr.width * 4);
    var c = (codigoqr.imagedata.data[a] * 33 + codigoqr.imagedata.data[a + 1] * 34 + codigoqr.imagedata.data[a + 2] * 33) / 100;
    return c
};
codigoqr.binarize = function(d) {
    var c = new Array(codigoqr.width * codigoqr.height);
    for (var e = 0; e < codigoqr.height; e++) {
        for (var b = 0; b < codigoqr.width; b++) {
            var a = codigoqr.getPixel(b, e);
            c[b + e * codigoqr.width] = a <= d ? true : false
        }
    }
    return c
};
codigoqr._em = function(d) {
    var c = 4;
    var k = Math.floor(codigoqr.width / c);
    var j = Math.floor(codigoqr.height / c);
    var f = new Array(c);
    for (var g = 0; g < c; g++) {
        f[g] = new Array(c);
        for (var e = 0; e < c; e++) {
    ...
    for (var o = 0; o < c; o++) {
        for (var a = 0; a < c; a++) {
            ...
            for (var l = 0; l < j; l++) {
                for (var n = 0; n < k; n++) {
                    var h = d[k * a + n + (j * o + l) * codigoqr.width];
                    if (h < f[a][o][0]) {
                        f[a][o][0] = h
                    }
                    if (h > f[a][o][1]) {
...
codigoqr.grayScaleToBitmap = function(f) {
    var k = codigoqr._em(f);
    var b = k.length;
    var e = Math.floor(codigoqr.width / b);
    var d = Math.floor(codigoqr.height / b);
    var h = new ArrayBuffer(codigoqr.width * codigoqr.height);
    var c = new Uint8Array(h);
    for (var j = 0; j < b; j++) {
        for (var a = 0; a < b; a++) {
            for (var g = 0; g < d; g++) {
                for (var i = 0; i < e; i++) {
                    c[e * a + i + (d * j + g) * codigoqr.width] = (f[e * a + i + (d * j + g) * codigoqr.width] < k[a][j]) ? true : false
                }
            }
        }
    }
...
codigoqr.grayscale = function() {
    var e = new ArrayBuffer(codigoqr.width * codigoqr.height);
    var c = new Uint8Array(e);
    for (var d = 0; d < codigoqr.height; d++) {
        for (var b = 0; b < codigoqr.width; b++) {
            var a = codigoqr.getPixel(b, d);
            c[b + d * codigoqr.width] = a
        }
    }
    return c
};
...
function _cc() {
    ...
    this._ap = function(a, j, d, g) {
        var c = this.image;
        var h = codigoqr.height;
        var b = this._da;
        var f = a;
        while (f >= 0 && c[j + f * codigoqr.width]) {
            b[2]++;
            f--
        }
        if (f < 0) {
        ...
        while (f >= 0 && !c[j + f * codigoqr.width] && b[1] <= d) {
            b[1]++;
            f--
        }
        if (f < 0 || b[1] > d) {
        ...
        while (f >= 0 && c[j + f * codigoqr.width] && b[0] <= d) {
            b[0]++;
            f--
        }
        if (b[0] > d) {
        ...
        f = a + 1;
        while (f < h && c[j + f * codigoqr.width]) {
            b[2]++;
            f++
        }
        if (f == h) {
        ...
        while (f < h && !c[j + f * codigoqr.width] && b[3] < d) {
            b[3]++;
            f++
        }
        if (f == h || b[3] >= d) {
        ...
        while (f < h && c[j + f * codigoqr.width] && b[4] < d) {
            b[4]++;
            f++
        }
        if (b[4] >= d) {
    ...
    this._ej = function(b, a, e, h) {
        var d = this.image;
        var i = codigoqr.width;
        var c = this._da;
        var g = b;
        while (g >= 0 && d[g + a * codigoqr.width]) {
            c[2]++;
            g--
        }
        if (g < 0) {
        ...
        while (g >= 0 && !d[g + a * codigoqr.width] && c[1] <= e) {
            c[1]++;
            g--
        }
        if (g < 0 || c[1] > e) {
        ...
        while (g >= 0 && d[g + a * codigoqr.width] && c[0] <= e) {
            c[0]++;
            g--
        }
        if (c[0] > e) {
        ...
        g = b + 1;
        while (g < i && d[g + a * codigoqr.width]) {
            c[2]++;
            g++
        }
        if (g == i) {
        ...
        while (g < i && !d[g + a * codigoqr.width] && c[3] < e) {
            c[3]++;
            g++
        }
        if (g == i || c[3] >= e) {
        ...
        while (g < i && d[g + a * codigoqr.width] && c[4] < e) {
            c[4]++;
            g++
        }
        if (c[4] >= e) {
    ...
    this._ce = function(e) {
        var o = false;
        this.image = e;
        var n = codigoqr.height;
        var k = codigoqr.width;
                        ...
                        if (b == 4) {
                            if (this._ao(d)) {
                                ...
                                if (c) {
                                    ...
                                    do {
                                        f++
                                    } while (f < k && !e[f + h * codigoqr.width]);
                                    f--
                                }
                                b = 0;
                                d[0] = 0;
...
function _ak(g, c, b, f, a, e, d) {
    ...
    this._ap = function(h, q, l, o) {
        var k = this.image;
        var p = codigoqr.height;
        var j = this._al;
        j[0] = 0;
        j[1] = 0;
        j[2] = 0;
        var n = h;
        while (n >= 0 && k[q + n * codigoqr.width] && j[1] <= l) {
            j[1]++;
            n--
        }
        if (n < 0 || j[1] > l) {
        ...
        while (n >= 0 && !k[q + n * codigoqr.width] && j[0] <= l) {
            j[0]++;
            n--
        }
        if (j[0] > l) {
        ...
        n = h + 1;
        while (n < p && k[q + n * codigoqr.width] && j[1] <= l) {
            j[1]++;
            n++
        }
        if (n == p || j[1] > l) {
        ...
        while (n < p && !k[q + n * codigoqr.width] && j[2] <= l) {
            j[2]++;
            n++
        }
        if (j[2] > l) {
    ...
    this.find = function() {
        ...
        for (var k = 0; k < s; k++) {
            ...
            m[2] = 0;
            var n = p;
            while (n < q && !g[n + codigoqr.width * o]) {
                n++
            }
            var h = 0;
            while (n < q) {
                if (g[n + o * codigoqr.width]) {
                    if (h == 1) {
                        m[h]++
                    } else {
                        if (h == 2) {
...
function QRCodeDataBlockReader(c, a, b) {
    ...
    this.getDataLength = function(d) {
        ...
        return this._gd(codigoqr._eo[this.dataLengthMode][e])
    };
